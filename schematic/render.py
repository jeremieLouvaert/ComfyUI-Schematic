...
from . import engine
from .draw_common import SS, FONT_REL_PATH, _load_font, _rgba, _dashed_line, _line_round_cap, _resolve_overlay
...
def apply_texture_screen(canvas01, texture01, opacity, mask01=None):
    ...
    # float64 promotion is load-bearing: a float32 mask would demote
    # opacity*mask to float32 (value-based casting) and shift the blend for any
    # opacity not exactly representable in float32 (e.g. 0.7), breaking the
    # ones-mask == no-mask byte guarantee.
    mask = mask01.astype(np.float64, copy=False)
    return base + (screen - base) * (opacity * mask[..., None])
...
def render_frame(fitted01, w, h, palette, params, elements, texture01, include_image, font_dir,
    ...
    if texture_opacity > 0:
        ...
        canvas = apply_texture_screen(canvas, tex, texture_opacity, texture_mask01)
